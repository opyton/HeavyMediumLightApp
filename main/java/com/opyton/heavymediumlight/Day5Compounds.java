...
public class Day5Compounds extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_day5_compounds);

        squat = getIntent().getStringExtra("squatNum");
        if(squat!=null){
            backoffSquat=Integer.parseInt(squat);
            double change = backoffSquat / 0.94 * .86;
            squat = Integer.toString((int)(5*(Math.floor(Math.abs(change/5)))));
            change = change*0.9;
            backoffSquat= (int) (5*(Math.floor(Math.abs(change/5))));
        }


        TextView topSquat = (TextView)findViewById(R.id.topSetDay5);
        TextView backoffSquatView = (TextView)findViewById(R.id.backoffSetDay5);
        ...
        Button nextBtn = (Button)findViewById(R.id.Day5NextCompound);
